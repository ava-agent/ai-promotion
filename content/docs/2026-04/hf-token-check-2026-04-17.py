...
def check_token_validity():
    ...
    try:
        ...
        print(f"✅ HF Token有效，用户: {user_info['name']} ({user_info['type']})")
        print(f"   - 用户ID: {user_info['id']}")
        print(f"   - 空间配额: {user_info.get('diskSpace', 0)} MB")
        print(f"   - 付费状态: {'✅ 付费' if user_info.get('jobId', 0) > 0 else '🆓 免费'}")
        ...
        try:
            ...
            if "IKUN-LLM" in org_names:
                ...
                try:
                    ...
                    print(f"⚠️ 获取组织模型失败: {e}")
                    
            else:
                print("❌ 无法访问IKUN-LLM组织")
                return False
...
def main():
    """主函数"""
    print("🚀 HF Token验证程序启动 - 2026-04-17")
    print("=" * 50)
    ...
    print("\n" + "=" * 50)
    if success:
        print("🎉 Token验证成功！系统运行正常")
        return 0
    else:
        print("💥 Token验证失败！需要检查配置")
        return 1
