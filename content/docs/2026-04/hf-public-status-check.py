...
try:
    ...
    if response.status_code == 200:
        print('✅ IKUN-LLM 组织页面可正常访问')
        # 提取模型数量
        if 'ikun-2.5B' in response.text:
            print('✅ ikun-2.5B 模型在组织页面可见')
        if 'model-card-data' in response.text:
            print('✅ 模型卡片数据存在')
    else:
        print(f'❌ 组织页面访问失败 ({response.status_code})')
except Exception as e:
    print(f'❌ 组织页面检查失败 ({str(e)})')
...
try:
    ...
    if response.status_code == 200:
        print('✅ ikun-2.5B 模型页面可正常访问')
        # 提取下载信息
        if 'downloads' in response.text.lower():
            print('✅ 下载信息显示正常')
    else:
        print(f'❌ 模型页面访问失败 ({response.status_code})')
except Exception as e:
    print(f'❌ 模型页面检查失败 ({str(e)})')
